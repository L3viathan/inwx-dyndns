...
def get_account_data(
    print_errors=False, config_file="python-inwx-xmlrpc.cfg", config_section="live"
):
    """
    ...
    try:
        ...
        message = (
            'Error: Please make sure your config file %s contains the section %s with the entries "url", "username" and "password".'
...
def get_domain_update(
    print_errors=False,
    config_file="python-inwx-xmlrpc.cfg",
    config_section="automatic_nameserver_entry_update",
):
    config = open_config_file(print_errors, config_file)
    try:
        ...
        message = (
            'Error: Please make sure your config file %s contains the section %s with the entries "domain", "subdomain" and "default_ip".'
            % (config_file, config_section)
        )
        if print_errors:
            print(message)
...
def get_invoices_folder(
    print_errors=False,
    config_file="python-inwx-xmlrpc.cfg",
    config_section="invoices_folder",
):
    config = open_config_file(print_errors, config_file)
    try:
        ...
        message = (
            'Error: Please make sure your config file %s contains the section %s with the entry "invoices_folder".'
            % (config_file, config_section)
        )
        if print_errors:
            print(message)
...
def get_nsbackup_files(
    print_errors=False,
    config_file="python-inwx-xmlrpc.cfg",
    config_section="nameserver_backup",
):
    config = open_config_file(print_errors, config_file)
    backup_files = dict()
    try:
        backup_files["json_backup_file"] = expanduser(
            config.get(config_section, "json_backup_file")
        )
    except:
        pass
    try:
        backup_files["pickle_backup_file"] = expanduser(
            config.get(config_section, "pickle_backup_file")
        )
    except:
        pass
    if len(backup_files) == 0:
        message = (
            'Error: Please make sure your config file %s contains the section %s with the entries "json_backup_file" or "pickle_backup_file".'
            % (config_file, config_section)
        )
        if print_errors:
            print(message)
...
def open_config_file(print_errors, config_file):
    ...
    try:
        ...
        message = (
            "Error: Please make sure you adopted the config file: %s" % config_file
        )
        if print_errors:
            print(message)
            sys.exit(2)
